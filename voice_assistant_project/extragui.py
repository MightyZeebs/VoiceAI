...
class WindowSpawner(BoxLayout):
    def spawn_assistant_window(self):
        existing_window = self.parent.ids["mainarea"].ids.get('AssistantWindow', None)
        if not existing_window:
            assistant_win = AssistantWindow()
            assistant_win.id = "AssistantWindow"
            self.parent.ids['mainarea'].add_widget(assistant_win)

    def spawn_search_results_window(self):
        existing_window = self.parent.ids["mainarea"].ids.get('SearchResultsWindow', None)
        if not existing_window:
            search_results_win = SearchResultsWindow()
            search_results_win.id = "SearchResultsWindow"
            self.parent.ids['mainarea'].add_widget(search_results_win)

class AssistantWindow(Widget):
    pass

class SearchResultsWindow(Widget):
    pass
...
Factory.register('WindowSpawner', cls=WindowSpawner)